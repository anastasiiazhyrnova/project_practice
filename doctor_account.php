...
$db = new ClinicDB();
$db->connect();
$sql = "select surname, name, patronymic, email from users where email = '{$_SESSION['login']}'";
$res = $db->makeQuery($sql);

$sql_doc = "select job from doctors where name like '{$res[0]['surname']}%'";
$res_doc = $db->makeQuery($sql_doc);

$sql_assignments = "select doctor, surname, name, patronymic, date from assignments where name_doctor like '{$res[0]['surname']}%'";
            ...
            <nav class="navbar navbar-expand-lg bg-white navbar-light py-3 py-lg-0">
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                    <div class="navbar-nav ms-auto py-0">
                        ...
                        <?php else : ?>
                            <a href="doctor_account.php" class="nav-item nav-link active">Мій акаунт</a>
                        <?php endif; ?>
                ...
                <table class="table align-middle mb-0 bg-white" id="le-Table-1">
                    <thead class="bg-light">
                        ...
                        <?php foreach ($res_assign as $row) : ?>
                            <tr>
                                <td class="fw-bold mb-1 text-center"><?= strval($row['surname']) . " " . strval($row['name']) . " " . strval($row['patronymic']); ?></td>
